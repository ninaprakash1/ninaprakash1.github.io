...
var text2 = d3.select('#text_vis3').attr('width',window.outerWidth).attr('height',400)
...
text2.append('text')
    .text('We often don’t think much about electricity until there is an outage. As our current power lines, which were')
    .attr('x', window.outerWidth / 2)
    .attr('y', 120)
    .attr('text-anchor','middle')

text2.append('text')
    .text('constructed in the 1950s and 60s, reach their age limitations and we face an increasing number of extreme weather')
    .attr('x', window.outerWidth / 2)
    .attr('y', 150)
    .attr('text-anchor','middle')

text2.append('text')
    .text('events due to climate change, ensuring that the grid can withstand unexpected disruptions or failures will be increasingly')
    .attr('x', window.outerWidth / 2)
    .attr('y', 180)
    .attr('text-anchor','middle')

text2.append('text')
    .text('important. The map below shows major grid outages for every month from 2000 to 2014 sized by the number')
    .attr('x', window.outerWidth / 2)
    .attr('y', 210)
    .attr('text-anchor','middle')

text2.append('text')
    .text(' of customers that were affected and colored by the cause of the outage. The scatter plot on the right shows another')
    .attr('x', window.outerWidth / 2)
    .attr('y', 240)
    .attr('text-anchor','middle')

text2.append('text')
    .text('view of the same data, where the x-axis represents time and the y-axis represents number of customers affected.')
    .attr('x', window.outerWidth / 2)
    .attr('y', 270)
    .attr('text-anchor','middle')

text2.append('text')
    .text('You can use the slider to move both the map and scatter plot through time. Observe that most of the outages are due')
    .attr('x', window.outerWidth / 2)
    .attr('y', 300)
    .attr('text-anchor','middle')

text2.append('text')
    .text('to severe weather events or equipment failure. This highlights the need to modernize energy system to deal with the')
    .attr('x', window.outerWidth / 2)
    .attr('y', 330)
    .attr('text-anchor','middle')

text2.append('text')
    .text('demands and climate risk of the 21st century.')
    .attr('x', window.outerWidth / 2)
    .attr('y', 360)
    .attr('text-anchor','middle')